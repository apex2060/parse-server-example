...
app.lazy.controller('AdminFormsFillCtrl', function($scope, $http, $timeout, $q, $routeParams, $interpolate, Parse, Auth, Google) {
	it.rp = $routeParams;
	var Forms = new Parse('Forms', true);
	var Data = null;
	$scope.data = {};
	...
	var tools = $scope.tools = {
			...
			save: function(){
				...
				tools.form.preSave(form, data).then(function(data){
					...
					$http.post('https://api.parse.com/1/functions/formSubmit', request).success(function(data){
						...
						var e = angular.fromJson(e.error)
						if(e.code == 111)
							toastr.error('Please contact the creator of this form:  '+e.message)
						else
		...
		item: {
			...
			cloudConvert: function(file){
				if(file && file.secure_url)
					return file.secure_url.replace('.pdf', '.jpg')
			},
			addCloud: function(item){
