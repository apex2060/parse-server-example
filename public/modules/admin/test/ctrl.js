app.lazy.controller('TestCtrl', function($rootScope, $scope, $routeParams, Parse, config){
	...
	var tools = $scope.tools = {
		init: function(list){
			if($routeParams.id){
				var Data = new Parse($routeParams.id)
				Data.list().then(function(list){
					tools.table.columns(list);
		...
		table: {
			columns: function(list){
				tools.object.keys(list).map(function(key){
					$scope.gridOptions.columnApi.addValueColumn({
						headerName: key,
						field: key,
						width: 150,
						filter: 'text'
